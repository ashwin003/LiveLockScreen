...
export default class PipelineUnsafe {
    ...
    init() {
        ...
        try {
            ...
            if (!this._videoBus || !this._videoSink)
                throw new Error('Failed to get video sink or bus');

            const interval = 1000 / this._framerate;
            this._timeoutId = this._startFetchTimer(interval);
            if (!this._timeoutId)
                throw new Error('Failed to create fetch timer');

            this._initBusWatch();
            this._initialized = true;
            return true;
        } catch(e) {
            console.error('PipelineUnsafe init failed:', e.message);
            this.deinit();
            return false;
        }
    }
